...
export class GISGeoPackageService {
  ...
  public static async getRoadsInBoundingBox(polygon: [number, number][]): Promise<{ nodes: Map<number, OSMNode>, ways: OSMWay[] }> {
    ...
    const [roadsErr, roads] = await safe(prisma.gISRoadData.findMany({
      where: {
    ...
    if (roadsErr || !roads) {
      console.error('[GISGeoPackageService] Failed to load roads from database. Returning empty.', roadsErr);
      return { nodes, ways };
    }
