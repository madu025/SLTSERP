...
export class LedgerService {
    static async logInvoiceApproval(tx: any, invoice: any) {
        // IFRS-15 Revenue Recognition and AR
        // Dr. AR (Accounts Receivable), Cr. Revenue
        const arAccount = '1100'; // Assuming AR account code
        const revAccount = '4000'; // Assuming Revenue account code
        
        const lines = [
            { accountCode: arAccount, debit: invoice.totalAmount, credit: 0, description: 'AR for Invoice ' + invoice.invoiceNumber },
            { accountCode: revAccount, debit: 0, credit: invoice.totalAmount, description: 'Revenue for Invoice ' + invoice.invoiceNumber }
        ];
        
        // Add retention if applicable
        if (invoice.retentionAmount > 0) {
            lines.push({ accountCode: '2100', debit: 0, credit: invoice.retentionAmount, description: 'Retention Payable for Invoice ' + invoice.invoiceNumber });
        }

        return await this.postTransaction(tx, {
            referenceId: invoice.id,
            referenceType: 'Invoice',
            description: 'Invoice Approval for ' + invoice.invoiceNumber,
            date: new Date(),
            lines
        });
    }
     ...
     * 1. Moves Unbilled WIP to AR
     * 2. Recognizes Contractor Payable (amountA) and Retention Liability (5%)
     */
    static async logInvoiceGeneration(
        ...
        // Move Unbilled WIP to AR (Revenue side)
        if (totalRevenue > 0) {
            lines.push(
                { accountCode: ACCOUNTS.AR_CLIENT, debit: totalRevenue, credit: 0, description: 'Accounts Receivable (Invoiced)' },
