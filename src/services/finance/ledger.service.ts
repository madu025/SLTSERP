...
export class LedgerService {
    ...
    static async postTransaction(
        ...
        // approvedById is a UUID FK to User — a system-posted entry has no human
        // approver. status=POSTED + isLocked already encode the auto-approval
        // semantics; a placeholder string ('SYSTEM_AUTO') violates the UUID column
        // and crashes every auto-generated posting (P2023).
    ...
    /**
     * Accrue WIP Liability upon SOD Completion
     */
    static async accrueWipLiability(sodId: string, tx: TransactionClient) {
        // Find SOD to get amounts and contractor
        ...
        if (!sod || !sod.contractorAmount) {
            console.log(`[LedgerService] Skipping WIP Accrual for SOD ${sodId} (No amount)`);
            return;
        }

        const amount = Number(sod.contractorAmount);

        return await this.postTransaction(tx, {
            referenceId: sodId,
            referenceType: 'SOD_WIP_ACCRUAL',
            description: `WIP Liability Accrual for SOD ${sod.soNum}`,
            isAutoGenerated: true,
            idempotencyKey: `WIP_ACCRUAL_${sodId}`,
            lines: [
                {
                    accountCode: ACCOUNTS.COGS,
                    debit: amount,
                    credit: 0,
                    description: `Cost of Goods Sold (Contractor Expense)`
                },
                {
                    accountCode: ACCOUNTS.AP_CONTRACTOR,
                    debit: 0,
                    credit: amount,
                    description: `Accrued WIP Liability (Contractor Payable)`
                }
            ]
        });
    }
