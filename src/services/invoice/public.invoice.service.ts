...
export class PublicInvoiceService {
    static async getPublicInvoiceDetails(id: string) {
        if (!id) {
        ...
        for (const sod of invoice.sods) {
            ...
            const dwUsage = sod.materialUsage.find((m: Record<string, unknown>) => {
                ...
                const usageType = String(m.usageType || '').toUpperCase();
        ...
        return {
            ...
            sods: invoice.sods.map((is: Record<string, unknown>) => ({
                serviceOrderId: is.id,
                soNum: is.soNum,
                rtom: is.rtom,
                serviceType: is.serviceType,
                voiceNumber: is.voiceNumber,
                circuitNumber: is.circuitNumber as string | undefined || is.voiceNumber || is.soNum,
                package: is.package,
                comments: is.comments,
                directTeam: is.directTeam,
                completedAt: is.completedDate,
                erectedPoles: is.erectedPoles || [],
                materialUsage: Array.isArray(is.materialUsage) ? is.materialUsage.map((mu: Record<string, unknown>) => ({
                    itemId: mu.itemId,
                    quantity: parseFloat(String(mu.quantity)),
