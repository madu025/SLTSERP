...
export default function VendorBulkImportPage() {
    ...
    const downloadTemplate = () => {
        const headers = ["Vendor_Code", "Vendor_Name", "Contact_Person", "Email", "Phone", "Address", "BR_Number", "Bank_Name", "Bank_Branch", "Account_No"];
        const csvContent = headers.join(",") + "\n" +
            "V001,ABC Suppliers,John Doe,john@abc.com,0771234567,Colombo,BR1234,Bank of Ceylon,Borella,12345678";
    ...
    const processImport = async () => {
        ...
        try {
            ...
            const vendorsData = dataRows.map(row => {
                ...
                return {
                    code: String(rowData['Vendor_Code']).trim(),
                    name: String(rowData['Vendor_Name']).trim(),
                    contactPerson: rowData['Contact_Person'] ? String(rowData['Contact_Person']).trim() : null,
                    ...
                    brNumber: rowData['BR_Number'] ? String(rowData['BR_Number']).trim() : null,
                    bankName: rowData['Bank_Name'] ? String(rowData['Bank_Name']).trim() : null,
                    bankBranch: rowData['Bank_Branch'] ? String(rowData['Bank_Branch']).trim() : null,
                    bankAccountNo: rowData['Account_No'] ? String(rowData['Account_No']).trim() : null,
                };
            });

            const res = await fetch('/api/admin/finance/vendors/import', {
                method: 'POST',
