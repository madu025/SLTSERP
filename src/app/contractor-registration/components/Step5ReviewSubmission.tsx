...
const ReviewSection = ({ icon: Icon, title, children }: ReviewSectionProps) => (
    <div className="space-y-6 p-8 rounded-[32px] border border-slate-200 bg-white shadow-sm transition-all hover:border-slate-300">
        <div className="flex items-center gap-4 pb-4 border-b border-slate-200">
            <div className="p-2.5 bg-slate-100 rounded-xl text-slate-900 border border-slate-200 shadow-inner">
...
const ReviewItem = ({ label, value }: { label: string; value?: string | null }) => (
    <div className="space-y-1.5 group">
        <span className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] block ml-0.5 opacity-70">{label}</span>
...
const ReviewDoc = ({ label, url }: { label: string; url?: string | null }) => (
    <div className="flex items-center justify-between p-5 rounded-2xl border border-slate-200 bg-slate-50/50 shadow-sm overflow-hidden group hover:bg-white hover:border-blue-200 transition-all h-[72px]">
        <div className="flex items-center gap-4">
            <div className="h-11 w-11 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-900 group-hover:text-blue-600 transition-all shadow-sm">
                {url?.match(/\.(pdf)$/i) ? <FileText className="w-6 h-6" /> : <ImageIcon className="w-6 h-6" />}
            </div>
            <div className="space-y-0.5">
                <span className="text-[9px] font-black text-slate-900 uppercase tracking-widest block opacity-60">Format: Official Specimen</span>
...
export function Step5ReviewSubmission() {
    ...
    return (
        <div className="space-y-12 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div className="text-center space-y-6 pb-4 border-b border-slate-200">
                ...
                <div className="space-y-2">
                    ...
                    <h3 className="text-3xl lg:text-4xl font-black text-slate-900 tracking-tighter uppercase">Enrollment Registry Review</h3>
                    <p className="text-sm text-slate-900 max-w-xl mx-auto leading-relaxed font-bold opacity-90 uppercase tracking-tight">
            ...
            <div className="relative overflow-hidden p-10 bg-slate-950 rounded-[40px] text-white shadow-2xl mt-12 animate-in slide-in-from-bottom-8 duration-1000 border-t-8 border-blue-600 ring-1 ring-white/10 group/dec">
                ...
                <div className="relative flex flex-col md:flex-row gap-10 items-start md:items-center">
                    ...
                    <div className="space-y-3 flex-1">
                        ...
                        <p className="text-[15px] lg:text-[16px] text-white leading-relaxed font-bold tracking-tight opacity-90">
