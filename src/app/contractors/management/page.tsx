...
export default function ContractorsPage() {
    ...
    const handleInviteSubmit = async () => {
        ...
        try {
            const res = await fetch('/api/admin/contractors/generate-link', {
                method: 'POST',
    ...
    const handleResendLink = async (id: string) => {
        try {
            const res = await fetch(`/api/admin/contractors/${id}/resend-link`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                                                ...
                                                <tr key={contractor.id} className="hover:bg-slate-50/50 transition-colors">
                                                    ...
                                                    <td className="text-center">
                                                        <button
                                                            ...
                                                            className="inline-flex items-center gap-1.5 text-[11px] font-extrabold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-2.5 py-1 rounded-md transition-all cursor-pointer shadow-2xs"
                                                            title="Click to Manage Contractor Teams & Manpower Setup"
                                                        >
                                                            <Users className="w-3.5 h-3.5 text-indigo-600" />
                                                            <span>{contractor._count?.teams || 0} Teams</span>
                                                        </button>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td className="text-right pr-6 py-2.5">
                                                        <div className="inline-flex items-center gap-1.5">
                                                            <Button
                                                                ...
                                                                className="h-7 px-2 text-[10px] font-extrabold text-indigo-600 border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700 rounded-lg transition-all flex items-center gap-1"
                                                                title="Manage Contractor Teams & Manpower Setup"
                                                            >
                                                                <Users className="w-3.5 h-3.5 text-indigo-600" />
