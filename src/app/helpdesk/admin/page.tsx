...
import TicketTable, { TicketSummary } from "@/components/helpdesk/TicketTable";
import TicketDetailsDialog, { TicketDetail } from "@/components/helpdesk/TicketDetailsDialog";
...
export default function HelpdeskAdminDashboard() {
  ...
  const fetchQueueData = React.useCallback(async () => {
    ...
    try {
      ...
      if (json.success) {
        let ticketList: TicketSummary[] = json.data.tickets || [];
        
        // Handle unassigned filter client side if backend didn't pluck it
        if (assignmentFilter === "UNASSIGNED") {
          ticketList = ticketList.filter((t: TicketSummary) => !t.assignedTo && !t.assignedToId);
        }
  ...
  }, [search, statusFilter, priorityFilter, assignmentFilter, user?.id]);
  ...
  const handleViewTicketDetails = async (ticket: TicketSummary) => {
    try {
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5">
              <Card className="bg-card/75 border-border/40 hover:-translate-y-0.5 transition-transform duration-300">
                <CardHeader className="p-3.5 pb-1">
                  <CardTitle className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Open Incidents</CardTitle>
                </CardHeader>
              ...
              <Card className="bg-card/75 border-border/40 hover:-translate-y-0.5 transition-transform duration-300">
                <CardHeader className="p-3.5 pb-1">
                  <CardTitle className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">In Progress</CardTitle>
                </CardHeader>
                <CardContent className="p-3.5 pt-0">
              ...
              <Card className="bg-card/75 border-border/40 hover:-translate-y-0.5 transition-transform duration-300">
                <CardHeader className="p-3.5 pb-1">
                  <CardTitle className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Waiting User/Parts</CardTitle>
                </CardHeader>
                <CardContent className="p-3.5 pt-0">
              ...
              <Card className="bg-card/75 border-border/40 hover:-translate-y-0.5 transition-transform duration-300 animate-pulse border-red-500/25">
                <CardHeader className="p-3.5 pb-1">
                  <CardTitle className="text-[10px] uppercase font-bold tracking-wider text-red-500">Unresolved Critical</CardTitle>
                </CardHeader>
                <CardContent className="p-3.5 pt-0">
              ...
              <Card className="bg-card/75 border-border/40 hover:-translate-y-0.5 transition-transform duration-300">
                <CardHeader className="p-3.5 pb-1">
                  <CardTitle className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground">Resolved Today</CardTitle>
                </CardHeader>
                <CardContent className="p-3.5 pt-0">
