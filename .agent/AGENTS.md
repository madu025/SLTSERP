...
13. **Clean & Concise Code Comments (Minimalist Style)**: Avoid redundant, overly narrative, essay-style, or verbose comments (`//`). Keep code self-documenting with clear variable and function names. Only use comments for strictly essential architectural warnings, complex algorithmic edge cases, or non-obvious business domain constraints. No obvious comments like `// loop through items` or multi-paragraph narrative essays.
14. **🎨 Mandatory UI High-Contrast & Anti-Washed-Out Text Mandate (Zero Invisible Text Tolerance)**:
   - **No Light-on-Light**: NEVER use light fonts (`text-white`, `text-slate-100/200/300`, `text-gray-100/200`, `#ffffff`, `#f8fafc`) on light backgrounds (`bg-white`, `bg-slate-50/100/200`, `#f8fafc`, `#f1f5f9`). Light surfaces (table headers, cards, modals, dropdowns, KPI cards) MUST use high-contrast dark text (`text-slate-900`, `text-slate-800`, `text-slate-700`, `#0f172a`).
   - **No Dark-on-Dark**: NEVER use dark fonts (`text-slate-800/900`, `text-black`, `#000000`) on dark backgrounds (`bg-slate-900/950`, `bg-black`, `#0b1329`). Dark surfaces MUST use bright, crisp text (`text-white`, `text-slate-100`, `text-cyan-400`).
   - **Table Header & Global CSS Conflict Immunity**: Base CSS in `globals.css` (e.g. `table th { background-color: #f8fafc !important; }`) will overwrite Tailwind header classes unless specifically handled. When creating dark table headers or modals, always explicitly lock background and text colors using high-specificity classes (e.g., `report-table-dark`, `dark-workstation-table`) or scoped inline styles with `!important` so headers are never blanched white with white text.
   - **WCAG Compliance**: All titles, headers, badges, and labels MUST satisfy minimum WCAG AA/AAA contrast ratios (4.5:1 minimum). Zero tolerance for washed-out, low-contrast, or unreadable text.
...
## 🏗️ The 100% Zero-Hardcoding FSM Mandate (Final Architecture)
