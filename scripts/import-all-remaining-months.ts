...
const reportsToImport = [
    { monthName: 'April 2026', fileName: 'FN April Per Line Material Report 2026.xlsx' },
    { monthName: 'May 2026', fileName: 'FN May Per Line Material Report-2026.xlsm' },
    { monthName: 'June 2026', fileName: 'FN June Per Line Material Report-2026.xlsm' }
];

const folderPath = `C:\\Users\\Prasad\\Downloads\\Invoice\\FN Material Reports-2026\\FN Material Reports-2026`;
...
async function importSingleMonthReport(monthName: string, fileName: string) {
    const fullPath = `${folderPath}\\${fileName}`;
    console.log(`\n==================================================`);
    console.log(`🚀 STARTING IMPORT FOR ${monthName.toUpperCase()}`);
    ...
    // 2. Read Excel Sheet
    console.log(`📖 Reading Excel workbook for ${monthName}...`);
    const buf = fs.readFileSync(fullPath);
    const wb = XLSX.read(buf, { type: 'buffer', cellDates: true });

    // Target sheet resolution: "SOFT" or "Material Report" or "Sheet1" or first sheet
    let targetSheetName = wb.SheetNames[0];
    if (wb.SheetNames.includes('SOFT')) targetSheetName = 'SOFT';
    else if (wb.SheetNames.includes('Material Report')) targetSheetName = 'Material Report';
    else if (wb.SheetNames.includes('Sheet1')) targetSheetName = 'Sheet1';

    console.log(`Using Excel sheet: "${targetSheetName}"`);
    const ws = wb.Sheets[targetSheetName];

    if (!ws) {
        console.error(`❌ Sheet "${targetSheetName}" not found in ${fileName}`);
        return;
    }

    const rawRows = XLSX.utils.sheet_to_json<any[]>(ws, { header: 1 });

    // Find header row dynamically (row index where "SOD NUMBER" or "SO NUMBER" is present)
    let headerRowIdx = 1;
    for (let r = 0; r < Math.min(5, rawRows.length); r++) {
        const row = rawRows[r];
        if (Array.isArray(row)) {
            const hasSodCol = row.some(cell => String(cell || '').toUpperCase().includes('SOD NUMBER') || String(cell || '').toUpperCase().includes('SO NUMBER'));
            if (hasSodCol) {
                headerRowIdx = r;
                break;
            }
    ...
    console.log(`Header row identified at index ${headerRowIdx}`);

    // Header column indices
    const colIdx = {
        soNum: headers.findIndex(h => h.toUpperCase().includes('SOD NUMBER') || h.toUpperCase().includes('SO NUMBER')),
    ...
    console.log(` Parsing and aggregating ${monthName} SOD rows...`);
    ...
    for (let i = headerRowIdx + 1; i < rawRows.length; i++) {
        ...
        const rowMaterials: { itemId: string; quantity: number; unit: string; usageType: string; unitPrice: number; comment?: string | null }[] = [];
        ...
        if (existingSod) {
            ...
            if (dropWireDistance > existingSod.dropWireDistance) {
                existingSod.dropWireDistance = dropWireDistance;
                existingSod.revenueAmount = revenueAmount;
                existingSod.contractorAmount = contractorAmount;
            }
        } else {
            sodMap.set(soNum, {
                soNum,
    ...
    console.log(`Total aggregated unique ${monthName} SOD rows to import: ${sodRows.length}`);

    // 3. Batch DB Execution using small chunks of 100 inside transactions
    ...
    for (let i = 0; i < sodRows.length; i += CHUNK_SIZE) {
        const batchSODData = sodRows.slice(i, i + CHUNK_SIZE);
        const batchSoNums = batchSODData.map(s => s.soNum);

        await prisma.$transaction(async (tx) => {
            const existingSODs = await tx.serviceOrder.findMany({
                where: { soNum: { in: batchSoNums } },
                select: { id: true, soNum: true }
            });

            const existingMap = new Map<string, string>();
            existingSODs.forEach(s => existingMap.set(s.soNum, s.id));

            const sodIdsForMaterialCleanup: string[] = [];

            // Process in inner chunks of 10
            for (let c = 0; c < batchSODData.length; c += 10) {
                const innerChunk = batchSODData.slice(c, c + 10);

                await Promise.all(innerChunk.map(async (sod) => {
        ...
        console.log(` Progress [${monthName}]: ${totalProcessed}/${sodRows.length} SODs imported (${Math.round((totalProcessed / sodRows.length) * 100)}%)`);
    }

    console.log(`\n✅ ${monthName.toUpperCase()} IMPORT COMPLETED!`);
    console.log(`  - Total SODs Processed: ${sodRows.length}`);
    console.log(`  - Updated: ${totalUpdated} | Created: ${totalCreated}`);
    console.log(`  - Teams Linked: ${totalTeamsLinked}`);
    console.log(`  - Material Usage Lines Written: ${totalMaterialLinesCreated}\n`);
}

async function runSequentialImports() {
    console.log('🚀 CONTINUOUS IMPORT OF REMAINING 2026 MATERIAL REPORTS (April, May, June)...');
    for (const report of reportsToImport) {
        try {
            await importSingleMonthReport(report.monthName, report.fileName);
        } catch (err) {
            console.error(`❌ Import failed for ${report.monthName}:`, err);
        }
    }
    console.log('\n==================================================');
    console.log('🎉 ALL REMAINING 2026 MATERIAL REPORTS IMPORTED!');
    console.log('==================================================\n');
    await prisma.$disconnect();
}

runSequentialImports().catch(console.error);
