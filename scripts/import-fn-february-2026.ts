...
const filePath = `C:\\Users\\Prasad\\Downloads\\Invoice\\FN Material Reports-2026\\FN Material Reports-2026\\FN February Per Line Material Report-2026.xlsm`;
...
async function importFebruaryMaterialReport() {
    console.log('🚀 Starting Refined February 2026 Material Report Import...');
    ...
    // Target sheet resolution: "Material Report" or "SOFT" or first sheet
    const targetSheetName = wb.SheetNames.includes('Material Report')
        ? 'Material Report'
        : (wb.SheetNames.includes('SOFT') ? 'SOFT' : wb.SheetNames[0]);

    console.log(`Using Excel sheet: "${targetSheetName}"`);
    const ws = wb.Sheets[targetSheetName];

    if (!ws) {
        throw new Error(`Sheet "${targetSheetName}" not found in February report workbook.`);
    }
    ...
    console.log(' Parsing and aggregating February SOD rows with refined F1/G1 Outdoor/Indoor Drop Wire & DW-RT Retainers...');
    ...
    console.log(`Total aggregated unique February SOD rows to import: ${sodRows.length}`);

    // 3. Batch DB Execution using small chunks of 10 inside transactions
    ...
    console.log('\n==================================================');
    console.log('🎉 REFINED FEBRUARY 2026 MATERIAL IMPORT COMPLETED!');
    console.log('==================================================');
...
importFebruaryMaterialReport().catch((err) => {
    console.error('❌ February Import Failed with Error:', err);
    process.exit(1);
});
