...
const filePath = `C:\\Users\\Prasad\\Downloads\\Invoice\\FN Material Reports-2026\\FN Material Reports-2026\\FN March Per Line Material Report-2026.xlsm`;
...
async function importMarchMaterialReport() {
    console.log('🚀 Starting Refined March 2026 Material Report Import...');
    ...
    // Target sheet resolution: "SOFT" or "Material Report" or first sheet
    const targetSheetName = wb.SheetNames.includes('SOFT')
        ? 'SOFT'
        : (wb.SheetNames.includes('Material Report') ? 'Material Report' : wb.SheetNames[0]);

    console.log(`Using Excel sheet: "${targetSheetName}"`);
    const ws = wb.Sheets[targetSheetName];

    if (!ws) {
        throw new Error(`Sheet "${targetSheetName}" not found in March report workbook.`);
    }
    ...
    console.log(' Parsing and aggregating March SOD rows with refined F1/G1 Outdoor/Indoor Drop Wire & DW-RT Retainers...');
    ...
    console.log(`Total aggregated unique March SOD rows to import: ${sodRows.length}`);
    ...
    console.log('\n==================================================');
    console.log('🎉 REFINED MARCH 2026 MATERIAL IMPORT COMPLETED!');
    console.log('==================================================');
...
importMarchMaterialReport().catch((err) => {
    console.error('❌ March Import Failed with Error:', err);
    process.exit(1);
});
