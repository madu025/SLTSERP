...
let cookie='';
function ap(p,b){return new Promise(r=>{const d=b?JSON.stringify(b):'';const headers={'Content-Type':'application/json'};if(d)headers['Content-Length']=Buffer.byteLength(d);if(cookie)headers['Cookie']=cookie;const q=http.request({hostname:'localhost',port:3000,path:p,method:b?'POST':'GET',headers},res=>{let y='';res.on('data',c=>y+=c);res.on('end',()=>{const sc=res.headers['set-cookie'];if(sc){cookie=(Array.isArray(sc)?sc:[]).map(c=>c.split(';')[0]).join('; ')}try{r({s:res.statusCode,d:JSON.parse(y)})}catch{r({s:res.statusCode,d:y})}})});if(d)q.write(d);q.end()})}
(async()=>{
const l=await ap('/api/login',{username:'admin',password:'Admin@123'});
console.log('Login status:',l.s,'| cookie set:',!!cookie);
const [pj,wf]=await Promise.all([ap('/api/projects/'+PID),ap('/api/projects/'+PID+'/workflow')]);
const p=pj.d;
console.log('Project API status:',pj.s,'| data type:',typeof pj.d);
console.log('Workflow API status:',wf.s,'| data type:',typeof wf.d);
const st=(wf.d&&wf.d.stages)?wf.d.stages:[];
console.log('===============================================================');
